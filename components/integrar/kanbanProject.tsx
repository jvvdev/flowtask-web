...
export function KanbanProject({ kanbanList, setKanbanList }: KanbanProjectProps) {
    ...
    const filteredKanbanList = kanbanList.filter((task) => task.title.toLowerCase().includes(searchQuery.toLowerCase()))
    ...
    function dragEndKanban(event: DragEndEvent) {
        if (!debounce) {
            ...
            toast.error("Você deve aguardar para fazer essa ação novamente")
        }
    }
                ...
                <div className="flex gap-2 items-center">
                    {selectedTask != "0" && (
                        <div className="flex gap-2">
                            ...
                            <AlertDialog>
                                <AlertDialogTrigger className="p-2 flex items-center justify-center gap-2 rounded-md text-sm font-semibold bg-yellow-500/15 dark:bg-yellow-500/20 hover:bg-yellow-500/20 dark:hover:bg-yellow-500/30 border border-yellow-500/20 text-yellow-500 cursor-pointer">
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>Aqui você pode modificar suas tarefas.</AlertDialogDescription>
                                    </AlertDialogHeader>
                                    ...
                                    <form onSubmit={handleSubmit(handleUpdateTask)}>
                                        <div className="space-y-4">
                                            ...
                                            <div className="space-y-2">
                                                <p className="text-sm flex items-center gap-2 dark:text-zinc-200/80">Descrição</p>
                                                <Input
                                                    placeholder="Digite aqui"
                                                    className="mb-2"
                                            ...
                                            <div className="space-y-2">
                                                <p className="text-sm flex items-center gap-2 dark:text-zinc-200/80">Prioridade</p>
                                                <div className="rounded-md border border-border bg-background px-3 py-1.5">
                                                    <select
                                                        className="w-full border-border bg-background text-sm outline-none"
                                            ...
                                            <div className="space-y-2">
                                                <p className="text-sm flex items-center gap-2 dark:text-zinc-200/80">Status</p>
                                                <div className="rounded-md border border-border bg-background px-3 py-1.5">
                                                    <select
                            ...
                            <AlertDialog>
                                <AlertDialogTrigger className="p-2 flex items-center justify-center gap-2 rounded-md text-sm font-semibold bg-red-500/15 dark:bg-red-500/20 hover:bg-red-500/20 dark:hover:bg-red-500/30 border border-red-500/20 text-red-500 cursor-pointer">
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>Você tem certeza de que deseja deletar essa tarefa?</AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                        <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                        <AlertDialogAction
                                            className="bg-red-800 hover:bg-red-700"
                                            onClick={() => kanbanService.deleteTask(selectedTask)}
                                        >
                                            Deletar tarefa
                                        </AlertDialogAction>
                                    </AlertDialogFooter>
                        ...
                        <Popover>
                            ...
                            <PopoverContent className="w-auto min-w-36 p-0" align="end">
                                <div className="space-y-1">
                                    <div className="text-xs px-3 pt-3 font-medium uppercase text-muted-foreground/60">Exibição</div>
                                    <div className="space-y-1 px-1 pb-1">
                                        <button
                                            className={`px-2 py-1 bg-zinc-200/0 w-full text-left ${filter == "kanban" ? "bg-zinc-400/5" : "hover:bg-zinc-200/5 cursor-pointer"} rounded-md duration-200`}
                                            onClick={() => setFilter("kanban")}
                                        >
                                            Kanban
                                        </button>
                                        <button
                                            className={`px-2 py-1 bg-zinc-200/0 w-full text-left ${filter == "list" ? "bg-zinc-400/5" : "hover:bg-zinc-200/5 cursor-pointer"} rounded-md duration-200`}
            ...
            {loading ? (
                ...
                paginatedData.length > 0 ? (
                    <div className="flex flex-col md:flex-row items-start gap-6 mt-6">
                        <DndContext onDragEnd={dragEndKanban} sensors={sensors} collisionDetection={closestCenter}>
                            <SortableContext items={kanbanList.map((task) => task.id_kanban)} strategy={verticalListSortingStrategy}>
                                <div className="duration-200 w-full bg-zinc-200/40 dark:bg-zinc-800/30 dark:border dark:border-zinc-700/20 rounded-xl p-4 space-y-4">
                                    <div className="flex flex-col items-center gap-3 text-lg font-bold">
                                        <div className="flex items-center w-full gap-2">
                                            ...
                                            <h1>A fazer</h1>
                                            <div className="text-sm h-5 w-5 flex items-center justify-center bg-zinc-400/40 dark:bg-zinc-700/70 text-zinc-800 dark:text-zinc-200/70 rounded-full">
                                                {kanbanList.filter((task) => task.status === "Pendente").length}
                                            </div>
                                        </div>
                                ...
                                <div className="duration-200 w-full bg-zinc-200/40 dark:bg-zinc-800/30 dark:border dark:border-zinc-700/20 rounded-xl p-4 space-y-4">
                                    <div className="flex flex-col items-center gap-3 text-lg font-bold">
                                        <div className="flex items-center w-full gap-2">
                                            ...
                                            <h1>Em progresso</h1>
                                            <div className="text-sm h-5 w-5 flex items-center justify-center bg-zinc-400/40 dark:bg-zinc-700/70 text-zinc-800 dark:text-zinc-200/70 rounded-full">
                                                {kanbanList.filter((task) => task.status === "Em progresso").length}
                                            </div>
                                        </div>
                                ...
                                <div className="duration-200 w-full bg-zinc-200/40 dark:bg-zinc-800/30 dark:border dark:border-zinc-700/20 rounded-xl p-4 space-y-4">
                                    <div className="flex flex-col items-center gap-3 text-lg font-bold">
                                        <div className="flex items-center w-full gap-2">
                                            ...
                                            <h1>Concluído</h1>
                                            <div className="text-sm h-5 w-5 flex items-center justify-center bg-zinc-400/40 dark:bg-zinc-700/70 text-zinc-800 dark:text-zinc-200/70 rounded-full">
                                                {kanbanList.filter((task) => task.status === "Concluída").length}
                                            </div>
                                        </div>
                ...
                <div className="mt-4">
                    <DndContext onDragEnd={dragEndList} sensors={sensors}>
                        <SortableContext items={paginatedData.map((task) => task.id_kanban)} strategy={verticalListSortingStrategy}>
                            <div className="overflow-x-auto">
                                <Table className="min-w-[1510px] table-fixed border-separate border-spacing-0 [&_tr:not(:last-child)_td]:border-b">
                                    <TableHeader>
                                        <DndContext onDragEnd={dragEndListHeader} sensors={sensors}>
                                            <TableRow className="hover:bg-transparent">
                                                ...
                                                >
                                                    {loading
                                                        ? null
                                                        : paginatedData.length > 0
                                                            ? listHeader.map((item) => <ListHeader key={item.id} taskID={item.id} name={item.name} />)
                                                            : ""}
                                                </SortableContext>
                                            </TableRow>
                                        </DndContext>
...
function ListTaskRow({
    ...
    listHeader,
    handleSelectTask,
}: ListTaskRowProps & { handleSelectTask: (data: KanbanTask | null) => void }) {
    const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: item.id_kanban })
        ...
        return (
            <TableCell>
                <p className="overflow-hidden whitespace-nowrap text-ellipsis">
                    {item.createdAt.toLocaleString().slice(8, 10) +
                        "/" +
                        item.createdAt.toLocaleString().slice(5, 7) +
                        "/" +
                        item.createdAt.toLocaleString().slice(0, 4) +
                        " às " +
                        item.createdAt.toLocaleString().slice(11, 16)}
                </p>
            </TableCell>
        )
    }
...
function KanbanTaskView({
    ...
    const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: item.id_kanban })
    ...
    return (
        <div
            ...
            {...attributes}
            {...listeners}
            className={`select-none w-full p-4 bg-white/80 backdrop-blur-sm border border-gray-200/60 dark:bg-zinc-900/80 dark:border-zinc-700/40 rounded-xl hover:shadow-lg hover:scale-[1.01] transition-all duration-200 ease-out hover:bg-white/90 dark:hover:bg-zinc-900/90 ${isDragging ? "rotate-1" : ""}`}
        >
            <div className="flex justify-between items-start">
                <div className="flex flex-col items-start w-full">
                    <span
                        className={`text-sm font-semibold px-2.5 py-1.5 rounded-md transition-all duration-200 ${item.status === "Concluída"
                            ? "text-emerald-700 bg-emerald-100 dark:text-emerald-300 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800/50"
                            : item.priority === 2
                                ? "text-red-700 bg-red-100 dark:text-red-300 dark:bg-red-900/30 border border-red-200 dark:border-red-800/50"
                                : item.priority === 1
                                    ? "text-amber-700 bg-amber-100 dark:text-amber-300 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800/50"
                                    : "text-green-700 bg-green-100 dark:text-green-300 dark:bg-green-900/30 border border-green-200 dark:border-green-800/50"
                            }`}
                    >
                        {item.status === "Concluída"
                            ? "Concluído"
                    ...
                    <h1 className="text-lg font-bold mt-3 text-gray-900 dark:text-gray-100 leading-tight">{item.title}</h1>
                </div>
                <DropdownMenu>
                    <DropdownMenuTrigger>
                        <Ellipsis
                            className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-zinc-800/60 transition-all duration-200 cursor-pointer text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
                            size={36}
                        />
                    </DropdownMenuTrigger>
                    <DropdownMenuContent className="bg-white/95 backdrop-blur-sm dark:bg-zinc-800/95 border border-gray-200/60 dark:border-zinc-700/60 rounded-xl p-2 shadow-xl">
                        <DropdownMenuLabel className="flex items-center justify-center gap-2">
                            <button
                                className="hover:bg-gray-100 dark:hover:bg-zinc-700/60 py-2.5 px-4 font-medium transition-all duration-200 rounded-lg w-full cursor-pointer text-gray-700 dark:text-gray-200"
                                tabIndex={0}
                                role="button"
                                onMouseDown={(e) => e.stopPropagation()}
                                onClick={() => {
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300 mt-2 leading-relaxed">{item.description}</p>
            <div className="flex flex-col gap-2 2xl:gap-0 2xl:flex-row 2xl:items-center justify-between mt-4 pt-3 border-t border-gray-100 dark:border-zinc-800">
                <div className="text-gray-500 dark:text-gray-400 text-sm 2xl:w-[80%]">
                    Criador: <span className="font-semibold text-gray-900 dark:text-gray-100">{item.createdBy}</span>
                </div>
                <div className="flex items-center gap-2 2xl:w-[40%] text-gray-500 dark:text-gray-400">
...
function KanbanEmptyState({ taskID }: { taskID: number }) {
    const { attributes, listeners, setNodeRef, transform, transition, isOver } = useSortable({ id: taskID })
    ...
    return (
        <div
            ...
            {...attributes}
            {...listeners}
            className={`w-full h-24 flex items-center justify-center rounded-xl border-2 border-dashed transition-all duration-300 ${isOver
                ? "border-blue-400 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-500"
                : "border-gray-300 dark:border-zinc-600 hover:border-gray-400 dark:hover:border-zinc-500 hover:bg-gray-50/50 dark:hover:bg-zinc-800/30"
                }`}
        >
            <div className={`text-center transition-all duration-300 ${isOver ? "scale-110" : ""}`}>
                <div
                    className={`w-8 h-8 mx-auto mb-2 rounded-full flex items-center justify-center transition-all duration-300 ${isOver ? "bg-blue-100 dark:bg-blue-900/40" : "bg-gray-100 dark:bg-zinc-700"
                        }`}
                >
                    <Plus
                        className={`w-4 h-4 transition-all duration-300 ${isOver ? "text-blue-600 dark:text-blue-400" : "text-gray-400 dark:text-zinc-500"
                            }`}
                    />
                </div>
                <p
                    className={`text-xs font-medium transition-all duration-300 ${isOver ? "text-blue-600 dark:text-blue-400" : "text-gray-400 dark:text-zinc-500"
                        }`}
                >
