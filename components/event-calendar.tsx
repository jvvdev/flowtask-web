...
export function EventCalendar({
  ...
  function capitalizeMonthYear(str: string) {
    return str.replace(/^\w+/, (m) => capitalize(m));
  }
  ...
  const viewTitle = useMemo(() => {
    if (view === "Mês") {
      ...
      if (isSameMonth(start, end)) {
        ...
        return `${capitalize(format(start, "MMMM", { locale: ptBR }))} - ${capitalizeMonthYear(
          format(end, "MMMM yyyy", { locale: ptBR })
        )}`;
      }
    } else if (view === "Dia") {
      return (
        <>
          <span className="min-sm:hidden" aria-hidden="true">
            {capitalize(format(currentDate, "MMM d, yyyy", { locale: ptBR }))}
          </span>
          <span className="max-sm:hidden min-md:hidden" aria-hidden="true">
            {capitalize(format(currentDate, "MMMM d, yyyy", { locale: ptBR }))}
          </span>
          <span className="max-md:hidden">
            {capitalize(format(currentDate, "EEE d", { locale: ptBR }))} de {capitalize(format(currentDate, "MMMM, yyyy", { locale: ptBR }))}
          </span>
        </>
      );
    } else if (view === "Agenda") {
