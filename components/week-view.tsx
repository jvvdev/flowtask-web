...
import { StartHour, EndHour, WeekCellsHeight } from "@/components/constants";
          ...
          if (col.length === 0) {
            ...
            const overlaps = col.some((c) =>
              areIntervalsOverlapping(
                ...
                {
                  start: new Date(c.event.initDate),
        ...
        {days.map((day) => (
          ...
          >
            <span className="max-sm:hidden">
              {format(day, "dd", { locale: ptBR })} {format(day, "EEEE", { locale: ptBR }).slice(0, 3)}
            </span>
            <span className="sm:hidden" aria-hidden="true">
              {format(day, "dd", { locale: ptBR })} {format(day, "EEE", { locale: ptBR })[0]}
            </span>
          </div>
        ))}
      </div>
        ...
        <div className="border-border/70 bg-muted/50 border-b">
          <div className="grid grid-cols-8">
            ...
            {days.map((day, dayIndex) => {
              const dayall_dayEvents = all_dayEvents.filter((event) => {
                const eventStart = new Date(event.initDate);
                const eventEnd = new Date(event.endDate);
                return (
                  isSameDay(day, eventStart) ||
